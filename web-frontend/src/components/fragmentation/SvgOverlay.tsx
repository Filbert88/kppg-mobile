...
interface SvgOverlayProps {
  ...
  /** When activeTool === "shapes", the prop shapeType determines whether to create a rectangle or a circle */
...
// Internal ephemeral state type for creation:
...
export function SvgOverlay({
  ...
  // Ephemeral creation state for new shapes or lines
  console.log("masuk", pointerEvents)
  ...
  function handlePointerDown(e: React.PointerEvent<SVGSVGElement>) {
    ...
    // 3) Creation: For "line", "rect", or "circle"
    if (activeTool === "line") {
      setCreatingShape({ type: "line", start: point, end: point });
      return;
    }
    if (activeTool === "shapes" ) {
    
      setCreatingShape({
        type: shapeType as "rect" | "circle",
        start: point,
        end: point
      });
      return;
    ...
    // 4) Selection mode when activeTool === "none"
    if (activeTool === "none") {
      ...
      // Tapped nothing: clear selection
  ...
  function handlePointerMove(e: React.PointerEvent<SVGSVGElement>) {
    ...
    // If creating a shape/line, update its end point
    if (creatingShape) {
        ...
        if (dragMode === "move") {
          setShapes((prev) =>
            prev.map((s) =>
              s.id === selectedElement.id
                ? { ...s, x: initialShape.x + dx, y: initialShape.y + dy }
                : s
            )
          );
          ...
          setShapes((prev) =>
            prev.map((s) => {
              ...
              switch (resizeCorner) {
                case "topLeft":
                  updated.width = s.width - dx;
                  updated.height = s.height - dy;
                  updated.x = initialShape.x + dx;
                  updated.y = initialShape.y + dy;
                  break;
                case "topRight":
                  updated.width = s.width + dx;
                  updated.height = s.height - dy;
          ...
          setLines((prev) =>
            prev.map((l) => {
              ...
              if (resizeLineEndpoint === "start") {
                updated.x1 = initialLine.x1 + dx;
                updated.y1 = initialLine.y1 + dy;
              } else {
                updated.x2 = initialLine.x2 + dx;
                updated.y2 = initialLine.y2 + dy;
              }
              return updated;
            })
          );
  ...
  // Ephemeral preview element during creation
  ...
  if (creatingShape) {
    ...
    if (type === "line") {
      ...
      const r =
        0.5 * Math.min(Math.abs(end.x - start.x), Math.abs(end.y - start.y));
      ephemeralEl = (
        <circle
          cx={cx}
          cy={cy}
