  ...
  export function SvgOverlay({
    ...
    function handlePointerDown(e: React.PointerEvent<SVGSVGElement>) {
      ...
      console.log("PointerDown at", point);
      ...
      // Creation: for line, or shapes (rect/circle)
      if (activeTool === "line") {
      ...
      // Selection mode (activeTool "none")
      if (activeTool === "none") {
        ...
        // Clear selection if tapped nothing
    ...
    function handlePointerMove(e: React.PointerEvent<SVGSVGElement>) {
      ...
      if (activeTool === "none" && selectedElement && dragMode) {
        ...
        if (selectedElement.type === "shape" && initialShape) {
          if (dragMode === "move") {
            // Move the shape but clamp within [0,width] and [0,height]
            let newX = initialShape.x + dx;
            let newY = initialShape.y + dy;
            newX = Math.max(
              0,
              Math.min(newX, width - Math.abs(initialShape.width))
            );
            newY = Math.max(
              0,
              Math.min(newY, height - Math.abs(initialShape.height))
            );
            setShapes((prev) =>
              prev.map((s) =>
            ...
            // Use the initial bounding box and the current pointer to compute new dimensions
            const { x1, y1, x2, y2 } = shapeBoundingBox(initialShape);
            const minSize = 10; // minimum allowed size
            let newX1 = x1,
              newY1 = y1,
              newX2 = x2,
              newY2 = y2;
