...
type NavigationProp = NativeStackNavigationProp<RootStackParamList, 'DatePriorityF'>;

const DatePriorityF = () => {
  const navigation = useNavigation<NavigationProp>();
  const {formData, resetForm, updateForm} = useContext(FormContext);
  ...
  const fetchNextPriority = async (date: string) => {
    ...
    if (isOnline) {
      try {
        const response = await fetch(`${API_BASE_URL}/api/Fragmentation/next-priority?tanggal=${date}`);
    ...
    <SafeAreaView className="flex-1">
      ...
      <View className="p-5 items-end">
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('FragmentationForm1')}>
